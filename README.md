...
| `apps/storybook` | `storybook`        | private | Storybook 10 configuration and the published playground; stories live in the package     |
...
Stories live beside the components they document, in `packages/whale`, so a new
component is one folder rather than an edit in two workspaces. `apps/storybook`
holds only the Storybook configuration; its `stories` glob points into the
package, and the stories themselves are excluded from the published build.
