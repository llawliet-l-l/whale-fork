...
rather than guessing from a listing. The dispatch itself needs no extra
credentials: `workflow_dispatch` is one of the few events the default
`GITHUB_TOKEN` may still trigger, so the job starts the child run on its own.
...
It is passed `channel: prod` explicitly because `deploy.yml` otherwise derives
the target from `github.ref`, which for a called workflow is the branch the
release was dispatched **from**, not `main`. That channel also implies the
checked-out ref, so there is nothing else to pass.
